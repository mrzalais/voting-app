<div>
    @auth
        <form wire:submit.prevent="createIdea" action="#" method="POST" class="space-y-4 px-4 py-6">
            ...
            <div>
                <select wire:model.defer="category" name="category_add" id="category_add"
                        class="w-full rounded-xl bg-gray-100 text-sm border-none px-4 py-2" required
                >
        ...
        <textarea wire:model.defer="description" name="idea" id="idea" cols="30" rows="4"
            ...
            <div class="flex items-center justify-between space-x-3">
                ...
                <button type="submit"
                        class="flex items-center justify-center w-1/2 h-11 text-xs bg-blue font-semibold rounded-xl
                        border border-blue text-white hover:bg-blue-hover transition duration-150 ease-in px-6 py-3">
    ...
    @else
        <div class="my-6 text-center">
            <a
                wire:click.prevent="redirectToLogin" action
                href="{{ route('login') }}" type="submit"
                class="inline-block justify-center w-1/2 h-11 text-xs bg-blue font-semibold rounded-xl border
                border-blue text-white hover:bg-blue-hover transition duration-150 ease-in px-6 py-3"
            >
                Login
            </a>
            <a
